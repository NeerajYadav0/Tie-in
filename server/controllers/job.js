...
// view applied job
const viewAppliedJob = async (req, res) => {
  try {
    ...
    if (userType === "student") {
      ...
      for (let x of student.job) {
       
        let job = await Job.findById(x.id);
        let result={"userStatus":x.status, ...job._doc};
...
// view created job
const viewCreatedJob = async () => {
  try {
    var data = [];
    const { userId } = req.body;
    const employer = await Employer.findById(userId);
    for (let x of employer.job) {
